...
def test_job_to_row_mapea_campos_fijos():
    ...
    row = job_to_row(job, snapshot_date=date(2026, 1, 1), run_id="run-1")
...
def test_load_vacio_no_toca_la_bd():
    res = load_jobs([], snapshot_date=date(2026, 1, 1), run_id="run-1")
...
def test_upsert_es_idempotente(db_conn, sample_jobs):
    ...
    r1 = load_jobs(sample_jobs, snapshot_date=snap, run_id="run-1", conn=db_conn)
    assert (r1.inserted, r1.updated) == (2, 0)

    r2 = load_jobs(sample_jobs, snapshot_date=snap, run_id="run-2", conn=db_conn)
